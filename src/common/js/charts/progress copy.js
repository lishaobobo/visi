...
import bac from "assets/images/group12.svg";
...
class Dashboard {
  ...
  init() {
    ...
    // 创建内圆
    if (options.showInnerCircle && !options.isDash) {
  ...
  createBackground() {
    const options = this.options;
    const baseAngle = ANGLE * options.allAngle;
    const offset = ANGLE * options.offsetAngle;
  ...
  createScale(count) {
    const { options } = this;
    const baseAngle = ANGLE * (options.allAngle / (count - 1));
    const fontSize = 16;
    const borderCenter = options.radius + fontSize * 2;
    ...
    this.textList.forEach((text, index) => {
      ...
      text
        .text(() => {
          return options.valueFormat ? options.valueFormat(_text) : _text * 10;
        })
        .attr(
          "x",
  ...
  createArc() {
    this.g
      ...
      .attr(
        "transform",
        `translate(${-(this.options.radius * 2) / 2},${-(
  ...
  createMarker() {
    ...
    //   .attr(
    //     "d",
    //     "M19.7275578,9.96152766 L32,31 L32,31 L4,31 L16.2724422,9.96152766 C16.8290028,9.00742383 18.0536382,8.68515134 19.0077421,9.24171191 C19.3057493,9.41554948 19.5537202,9.66352039 19.7275578,9.96152766 Z"
    //   )
    //   .attr("fill", "#f00")
    //   .attr("class", "arrow");
  ...
  render(value) {
    ...
    const options = this.options;
    const count = options.splitConut;
    const baseAngle = ANGLE * (options.allAngle / count);
    const offset = ANGLE * options.offsetAngle;
    ...
    //创建虚拟轨道以便arrow跟随
    if (options.isDash) {
      const total = options.offsetAngle + 15 - options.allAngle * value;
      const arrowRadius = options.radius - 30;
      const x = Math.sin(ANGLE * total) * arrowRadius;
      const y = Math.cos(ANGLE * total) * arrowRadius;

      this.marker.attr(
        "transform",
        `translate(${x},${y}) rotate(${value * options.allAngle -
          options.allAngle / 2})`
      );
    }
    ...
    if (value > 0 && value < 0.49) {
      options.textStyle.color = "#0f0";
    }
    if (value >= 0.5 && value < 0.74) {
      options.textStyle.color = "#0ff";
    }
    if (value >= 0.74) {
      options.textStyle.color = "#f00";
    }
    // 控制中心文字
  ...
  updateSize() {
    ...
    options.radius =
      minRadius / 2 - options.borderWidth - options.padding - fontSize;
