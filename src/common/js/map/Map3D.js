...
import Box from "./Map3D.layer";
import Ring from "./Map3D.Ring";
/**
 * 地图立体参数设置
 * @type {{thickness: number, bevelThickness: number, bevelSize: number, bevelEnabled: boolean, bevelSegments: number, curveSegments: number, steps: number}}
 */
var extrudeOption = {
  depth: 1, //厚度
  bevelThickness: 1,
...
class Map3D {
  constructor(o) {
    ...
    let opt = {
      ...
      autoRotate: false, //是否自动旋转视角
      ambientColor: 0x333333, //环境光颜色
      directionalColor: 0xaaaaaa, //平行光颜色
      hasLoadEffect: false, //是否有加载效果
      debugger: false, //调试模式
      cameraPosition: { x: 0, y: 0, z: 40 }, //相机位置
      extrude: extrudeOption, //立体厚度参数
  ...
  init() {
    ...
    this.scene.add(new THREE.AmbientLight(util.colorToHex(this.ambientColor)));
    this.dirLight = new THREE.DirectionalLight(
      util.colorToHex(this.directionalColor)
    );
    ...
    this.spotLight = new THREE.SpotLight(util.colorToHex(this.spotLightColor));
  ...
  inintDataRange(dataRangeOpt) {
    ...
    let dataRangeOptClone = Object.assign(
      { extrudeHeight: this.extrude.depth / 2 },
      this.dataRange
    );
  ...
  initMark(markOpt) {
    ...
    let markClone = Object.assign(
      { extrudeHeight: this.extrude.depth },
      this.mark
    );
   ...
   */
  initRing(ringOpt) {
    Object.assign(this.ring, ringOpt);
    let ringClone = Object.assign(
      { extrudeHeight: this.extrude.depth },
      this.ring
    );
    delete ringClone.data;
    Ring.count = 0;
    if (this.ringGroup) this.ringGroup.remove(...this.ringGroup.children);
    this.ringGroup = new THREE.Group();
    this.ring.data.forEach(userData => {
      let opt = Object.assign({}, userData, ringClone);
      let ring = new Ring(opt);
      this.ringGroup.add(ring);
    });
    this.mapObject.add(this.ringGroup);
  }
  ...
  setCameraPosition(v3, time, delay, easing, callback) {
    if (time && typeof time === "number")
      util.transition(this.camera.position, v3, time, delay, easing, callback);
   ...
   *
   * 删除圆环
   * @param {string | Mark} ring - 要删除的圆环名或者圆环对象
   */
  removeRing(ring) {
    if (typeof ring === "string") ring = this.getRing(ring);
    this.ringGroup.remove(ring);
  }
  /**
   *
   * 得到圆环对象
   * @param {string} ringName - 圆环名称
   * @returns {Ring}
   *
   */
  getRing(ringName) {
    return this.ringGroup.getObjectByName(ringName);
  }
  /**
   * 得到地图区域
  ...
  renderScene() {
    ...
    // 更新mark状态
    // this.markGroup.children.forEach(mark=>{
    //   mark.update()
    // })
    ...
    if (intersects.length > 0) {
      ...
      for (let i = 0; i < intersects.length; i++) {
        if (
          ...
          intersects[i].object.parent.type === "Mark"
        ) {
          this.selectedObj = intersects[i].object.parent;
          break;
          // } else if (
