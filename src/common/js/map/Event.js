...
export default class Event {
  constructor(o) {
    ...
    if (!this.camera && !this.renderer && !this.el) {
      console.warn(
        "THREE.Event:not find camera, renderer, domElement,Please ensure that it is a valid 3D control"
      );
      return;
    }
    if (!this.__resize && !this.__mousemove && !this.__mousedown) {
      console.warn(
        "THREE.Event:Users need to implement _onResize(),_onMouseMove(),_onMouseDown() method"
      );
      return;
    }
  ...
  onDocumentMouseMove(event) {
    ...
    this.mousemove_timer = setTimeout(() => {
      ...
      let intersects = this.__raycaster.intersectObjects(
        this.scene.children,
        true
      );
  ...
  onDocumentMouseDown(event) {
    event.preventDefault();
    this.__mouse.x =
      ((event.offsetX || event.clientX) / this.el.offsetWidth) * 2 - 1;
    this.__mouse.y =
      -((event.offsetY || event.clientY) / this.el.offsetHeight) * 2 + 1;
    this.__raycaster.setFromCamera(this.__mouse, this.camera);
    let intersects = this.__raycaster.intersectObjects(
      this.scene.children,
      true
